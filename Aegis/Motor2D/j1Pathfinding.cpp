...
int j1PathFinding::CreatePath(const iPoint& origin, const iPoint& destination)
{
	...
	while (open.list.count() > 0) {
		...
		p2List_item<PathNode>* new_node = closed.list.add(lowest_node->data);
		open.list.del(lowest_node);

		if (new_node->data.pos == destination)
		{
			last_path.Clear();

			const PathNode* path_node = &new_node->data;
		...
		PathList neighbours;
		new_node->data.FindWalkableAdjacents(neighbours);
